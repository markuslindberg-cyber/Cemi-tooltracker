  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Förflyttningar och lån</h1>
          <p className="text-gray-500 mt-1">
            {transfers.length} förflyttningar, {loanRequests.length} låneförfrågningar
          </p>
        </div>
        ...
        <Tabs defaultValue="transfers" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="transfers">Förflyttningshistorik</TabsTrigger>
            <TabsTrigger value="loans">Låneförfrågningar</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="loans" className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1 max-w-md">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                  <Input placeholder="Sök på maskin, plats eller person..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-11 border-gray-200" />
                </div>
                <Select value={loanStatusFilter} onValueChange={setLoanStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="rejected">Avslad</SelectItem>
                    <SelectItem value="returned">Återlämnad</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            {filteredLoans.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
                ...
                <h3 className="font-semibold text-gray-900 mb-2">
                  {loanRequests.length === 0 ? 'Inga låneförfrågningar ännu' : 'Inga matchande låneförfrågningar'}
                </h3>
                <p className="text-gray-500">
                  {loanRequests.length === 0 
                    ? 'Låneförfrågningar visas här när de skapas'
                    ...
                    <div key={loan.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <p className="text-sm text-gray-500 mb-1">Maskiner</p>
