...
import { Loader2, RefreshCw, Pencil, Check, X, ChevronDown, ChevronRight, Tag, Layers } from 'lucide-react';
...
function EditableField({ value, onSave, className = '' }) {
...
function CategoryRow({ category, onUpdateName, onUpdateSubcat }) {
    ...
    <div className="border border-gray-200 rounded-xl bg-white overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 font-medium text-gray-900" onClick={e => e.stopPropagation()}>
          <EditableField
            value={category.name}
            onSave={(newName) => onUpdateName(category, newName)}
          />
        </div>
        ...
        {category.page_label && (
          <span className="text-xs text-gray-400 hidden sm:block">📍 {category.page_label}</span>
        )}
            ...
            <div className="flex flex-wrap gap-2">
              {category.subcategories.map((sub, idx) => (
                <div key={idx} className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-sm">
                  ...
                  <EditableField
                    value={sub}
                    onSave={(newSub) => onUpdateSubcat(category, sub, newSub)}
                  />
                </div>
              ))}
            </div>
...
export default function CategoryManagement() {
  ...
  const handleUpdateName = async (category, newName) => {
    try {
      ...
      toast.success(`Kategori uppdaterad! ${res.data.updatedCount} artiklar/verktyg påverkade.`);
      queryClient.invalidateQueries(['categories']);
    } catch (err) {
  ...
  const handleUpdateSubcat = async (category, oldSubcat, newSubcat) => {
    try {
      ...
      toast.success(`Underkategori uppdaterad! ${res.data.updatedCount} artiklar/verktyg påverkade.`);
      queryClient.invalidateQueries(['categories']);
    } catch (err) {
      toast.error('Fel: ' + err.message);
    }
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800">
        <strong>Tips:</strong> Klicka på "Synkronisera kategorier" för att hämta alla kategorier från befintliga artiklar och verktyg i systemet. Klicka sedan på ett kategori- eller underkategorinamn för att redigera det – ändringen uppdateras automatiskt på alla ställen i appen.
      </div>
